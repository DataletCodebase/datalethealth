...
const cols = {
  ...
  Contact: ["support@datalet.in", "Agartala, Tripura, India"],
  ...
  return (
    <footer className="bg-white border-t border-[#8B00DC]/[0.08]">
      ...
      <div className="border-b border-[#8B00DC]/[0.06]">
        <div className="max-w-7xl mx-auto px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="font-bold text-[#0D0D0D] mb-1">
              Stay ahead of healthcare trends
            </div>
            <p className="text-[#9898A8] text-sm">
              Weekly insights from the Datalet team.
            </p>
          </div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <motion.button
              whileHover={{ scale: 1.04, y: -1 }}
              whileTap={{ scale: 0.97 }}
              className="px-6 py-3 text-white text-sm font-semibold rounded-2xl flex items-center gap-2 shadow-lg shadow-[#8B00DC]/20"
              style={{
      ...
      <div className="max-w-7xl mx-auto px-8 py-20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-[#9898A8] text-sm leading-relaxed mb-8 max-w-xs">
              Empowering healthcare organisations with data intelligence that
              drives better outcomes, reduced costs, and improved patient
              experiences.
            </p>
            ...
            <div className="flex gap-3">
              {/* Facebook */}
              <motion.a
                href="https://www.facebook.com/share/1CJoZYpnmq/"
                target="_blank"
                whileHover={{
                  scale: 1.15,
                  y: -2,
                  color: "#8B00DC",
                  borderColor: "rgba(139,0,220,0.3)",
                }}
                className="w-10 h-10 rounded-2xl border border-[#E8E0F5] flex items-center justify-center text-[#C4B8D8] transition-all"
              >
              ...
              {/* Instagram */}
              <motion.a
                href="https://www.instagram.com/datalethealthcare"
                target="_blank"
                whileHover={{
                  scale: 1.15,
                  y: -2,
          ...
          {Object.entries(cols).map(([title, links]) => (
            <div key={title}>
              <div className="text-[#0D0D0D] text-sm font-semibold mb-6">
                {title}
              </div>
                ...
                {links.map((link) => (
                  <li key={link}>
                    <motion.a
                      ...
                      onClick={() =>
                        title === "Quick Links"
                          ? go("#" + link.toLowerCase().replace(/\s/g, ""))
                          : null
                      }
                      whileHover={{ x: 3, color: "#8B00DC" }}
                      className="text-sm text-[#9898A8] transition-colors block cursor-pointer"
                    >
                      {link}
                    </motion.a>
                  </li>
      ...
      <div className="border-t border-[#8B00DC]/[0.06]">
        <div className="max-w-7xl mx-auto px-8 py-6 flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="text-[#C4B8D8] text-sm">
            © 2026 Datalet Healthcare, India. All rights reserved.
          </div>
