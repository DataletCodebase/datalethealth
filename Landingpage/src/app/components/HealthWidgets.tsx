...
import doctorPhoto from "../../assets/images/doctor1.avif";
import doctorPhoto1 from "../../assets/images/doctor2.avif";
...
function HealthScoreGauge({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center mt-2">
        <span className="font-black text-xl text-[#0D0D0D] leading-none">
          {Math.round(progress)}
        </span>
        <span className="text-[#9898A8] text-[9px] leading-none mt-0.5">
          / 100
        </span>
        <span className="text-[#8B00DC] text-[9px] font-semibold mt-1">
          Good
        </span>
      </div>
    </div>
...
function AnimBar({ value, color }: { value: number; color: string }) {
  ...
  return (
    <div
      ref={ref}
      className="flex-1 h-1.5 rounded-full bg-[#F0EAF8] overflow-hidden"
    >
      <motion.div
        className="h-full rounded-full"
        style={{ background: color }}
...
export function HealthDashboardWidget() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="text-[#9898A8] text-[11px] font-medium">
            Good Morning, Alex!
          </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="flex-1 space-y-2.5">
          {metrics.map((m) => (
            <div key={m.label} className="flex items-center gap-2">
              <m.icon className="w-3 h-3 shrink-0" style={{ color: m.color }} />
              <span className="text-[#5A5A72] text-[10px] w-14 shrink-0">
                {m.label}
              </span>
              <AnimBar value={m.bar} color={m.color} />
              <span className="text-[10px] font-semibold text-[#0D0D0D] w-7 text-right shrink-0">
                {m.value}
              </span>
            </div>
          ))}
      ...
      {/* Mini chart */}
      <div className="relative">
        <div className="text-[#9898A8] text-[10px] font-medium mb-2">
          Weekly Progress
        </div>
        <div className="relative" style={{ width: "100%", height: "52px" }}>
          <ResponsiveContainer width="100%" height={52}>
        ...
        <div className="flex justify-between mt-1.5">
          {weekData.map((d) => (
            <span
              key={d.day}
              className={`text-[8px] ${d.day === "Today" ? "text-[#8B00DC] font-bold" : "text-[#C4B8D8]"}`}
            >
              {d.day === "Today" ? "Now" : d.day.slice(0, 1)}
            </span>
          ))}
        </div>
      </div>
...
export function FoodAnalysisWidget() {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <div className="text-[#0D0D0D] font-bold text-sm mb-3">
          Food Analysis
        </div>
        <div className="grid grid-cols-3 gap-2">
          {macros.map((m, i) => (
            ...
            >
              ...
              <div className="text-[#9898A8] text-[9px] mt-0.5">{m.unit}</div>
              <div className="text-[#5A5A72] text-[9px] font-medium mt-1">
                {m.label}
              </div>
            </motion.div>
          ))}
...
// ── AI Wellness Results Widget ───────────────────────────────────────────────
const clinicianAvatar = doctorPhoto;
// "https://images.unsplash.com/photo-1650784854452-6f18e97221f4?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=200";
...
export function WellnessResultsWidget() {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5 mb-4">
        {vitals.map((v, i) => (
          ...
          >
            ...
            <span className="text-[#5A5A72] text-[11px] w-16 shrink-0">
              {v.label}
            </span>
            <div className="flex-1 h-1 rounded-full bg-[#F0EAF8]">
              <motion.div
      ...
      <div className="bg-[#FAF7FF] rounded-2xl p-3 border border-[#E8E0F5]">
        <div className="text-[#0D0D0D] text-[11px] font-semibold mb-1">
          Wellness Summary
        </div>
        <p className="text-[#5A5A72] text-[10px] leading-relaxed">
          You're doing well! Increase water intake to 8 glasses and aim for 7+
          hrs sleep tonight.
        </p>
      </div>
    </motion.div>
          ...
          <motion.div
            ...
            className={`flex items-center gap-3 p-2.5 rounded-2xl border transition-all ${
              item.done
                ...
                : "border-[#E8E0F5] hover:border-[#8B00DC]/20"
            }`}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[#0D0D0D] text-[11px] font-semibold">
                {item.label}
              </div>
              <div className="text-[#9898A8] text-[9px]">{item.detail}</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-[#9898A8] text-[9px]">{item.time}</div>
              {item.done && (
                <CheckCircle2 className="w-3 h-3 text-[#10B981] ml-auto mt-0.5" />
              )}
            </div>
          </motion.div>
        ))}
...
// ── Expert Review Widget ──────────────────────────────────────────────────────
const expertAvatar = doctorPhoto1;
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <div className="text-[#9898A8] text-[10px]">
      ...
      <div className="text-[#5A5A72] text-[11px] font-semibold mb-2.5">
        Recommendations
      </div>
      <div className="space-y-2">
        {[
      ...
      <div className="mt-4 pt-3 border-t border-[#E8E0F5]">
        <div className="text-[#C4B8D8] text-[9px] italic">
          Digitally signed by Dr. A. Sharma, PhD
        </div>
      </div>
    </motion.div>
  );
