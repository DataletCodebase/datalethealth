...
const clinicianAvatars = [doctor1, doctor2, doctor3, doctor4];

const testimonials = [
  {
    quote:
      "Datalet completely changed how we use clinical data. In six months we reduced preventable readmissions by 28% — and our care teams are spending more time with patients, not paperwork.",
  ...
  {
    quote:
      "The interoperability capabilities alone were worth it. We connected 14 disparate systems into one unified view. Our clinical staff finally has the complete picture they need at the point of care.",
    name: "James Okafor",
    title: "VP of Health IT",
    org: "St. Luke's Health System",
  },
  {
    quote:
      "Population health management used to feel impossible with our fragmented data. Datalet gave us the tools to identify care gaps at scale and reach patients proactively.",
    name: "Dr. Lisa Chen",
    title: "Director of Population Health",
    org: "Pacific Coast Medical Group",
  },
  {
    quote:
      "Datalet stands out for their clinical depth and implementation support. They're not just a technology company — they're a healthcare company that does technology.",
    name: "Michael Torres",
    title: "Chief Operating Officer",
    org: "Valley Health Partners",
  },
];
...
export function Testimonials() {
  ...
  return (
    <section
      id="testimonials"
      className="bg-[#FAF7FF] py-16 sm:py-24 lg:py-36 relative overflow-hidden"
    >
      <motion.div
        animate={{ scale: [1, 1.1, 1], opacity: [0.06, 0.1, 0.06] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[500px] rounded-full blur-[160px] pointer-events-none"
        style={{
          background: "radial-gradient(circle, #AA00FF, transparent 60%)",
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row items-end justify-between gap-8 mb-16">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-black text-[#0D0D0D] leading-tight">
              What our
              <br />
              customers say
            </h2>
          ...
          <div className="grid lg:grid-cols-[1fr_240px] gap-14 items-center">
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <div className="flex gap-1 mb-8">
                  {[...Array(5)].map((_, i) => (
                    ...
                    >
                      <Star
                        className="w-4 h-4"
                        style={{ fill: "#AA00FF", color: "#AA00FF" }}
                      />
                    </motion.span>
                  ))}
                </div>
                <div
                  className="text-5xl sm:text-8xl font-black leading-none mb-4 opacity-10"
                  style={{
                ...
                <p className="text-xl lg:text-2xl text-[#0D0D0D] leading-relaxed -mt-6 sm:-mt-10 mb-12">
                  {t.quote}
                </p>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative shrink-0">
                    <div
                      ...
                      style={{
                        background:
                          "radial-gradient(circle, #AA00FF, transparent 70%)",
                      }}
                    />
                    <div className="relative w-14 h-14 rounded-full overflow-hidden border-2 border-[#8B00DC]/30">
                      <img
                        src={clinicianAvatars[active]}
                        alt={t.name}
                  ...
                  <div>
                    <div className="font-bold text-[#0D0D0D]">{t.name}</div>
                    <div className="text-[#9898A8] text-sm">{t.title}</div>
                    <div className="text-sm font-semibold mt-0.5 text-[#8B00DC]">
                      {t.org}
                    </div>
                  </div>
                </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="relative">
                  ...
                  />
                  <div
                    className="relative w-full rounded-3xl overflow-hidden border border-[#E8E0F5] shadow-xl"
                  ...
                  >
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      <div className="bg-white/90 backdrop-blur-md border border-[#8B00DC]/15 rounded-2xl px-4 py-3 text-center shadow-lg">
                        ...
                        <div className="text-[#9898A8] text-xs">
                          Satisfaction
                        </div>
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {testimonials.map((tt, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className={`relative bg-white border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 shadow-sm group ${
                i === active
                  ? "border-[#8B00DC]/40 shadow-lg shadow-[#8B00DC]/20 scale-[1.02]"
                  : "border-[#E8E0F5] hover:border-[#8B00DC]/30"
              }`}
            >
              <div className="h-48 relative overflow-hidden rounded-t-2xl">
                <img
                  src={clinicianAvatars[i]}
                  alt={tt.name}
                ...
                {/* {i === active && (
                  <motion.div
                    ...
                    style={{
                      background:
                        "linear-gradient(to bottom, #AA00FF, transparent 60%)",
                    }}
                  />
                )} */}
              </div>
              <div className="p-4">
                <div className="text-[#0D0D0D] text-xs font-semibold leading-snug">
                  {tt.name}
                </div>
                <div className="text-[#9898A8] text-xs mt-0.5">{tt.title}</div>
                <div
                  className="text-xs font-medium mt-1 transition-colors"
                  style={{ color: i === active ? "#8B00DC" : "transparent" }}
                >
                  {tt.org}
                </div>
              </div>
