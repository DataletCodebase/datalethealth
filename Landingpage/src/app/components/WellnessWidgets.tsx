...
function ecgSample(t: number): number {
  ...
  v += 0.16 * Math.exp(-Math.pow((c - 0.12) / 0.026, 2));
  v -= 0.09 * Math.exp(-Math.pow((c - 0.382) / 0.009, 2));
  v += 1.0 * Math.exp(-Math.pow((c - 0.418) / 0.013, 2));
  v -= 0.28 * Math.exp(-Math.pow((c - 0.454) / 0.011, 2));
  v += 0.03 * Math.exp(-Math.pow((c - 0.53) / 0.04, 2));
  v += 0.26 * Math.exp(-Math.pow((c - 0.65) / 0.05, 2));
  return v;
}
...
function HeartRateWidget() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setTrend(() =>
        next > bpmRef.current ? "up" : next < bpmRef.current ? "down" : "flat",
      );
      setBpm(next);
      setPulse(true);
  ...
  useEffect(() => {
    ...
    function tick(ts: number) {
      ...
      for (let xi = 0; xi <= Math.ceil(W / minor); xi++) {
        const x = xi * minor;
        ctx.strokeStyle =
          xi % 5 === 0 ? "rgba(180,150,220,0.25)" : "rgba(200,180,230,0.11)";
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, H);
        ctx.stroke();
      ...
      for (let yi = 0; yi <= Math.ceil(H / minor); yi++) {
        const y = yi * minor;
        ctx.strokeStyle =
          yi % 5 === 0 ? "rgba(180,150,220,0.25)" : "rgba(200,180,230,0.11)";
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(W, y);
        ctx.stroke();
  ...
  const trendColor =
    trend === "up" ? "#ef4444" : trend === "down" ? "#22c55e" : "#9898A8";
      ...
      >
        <AnimatePresence>
          {pulse && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(circle at 50% 50%, rgba(239,68,68,0.18), transparent 70%)",
              }}
            />
          )}
        </AnimatePresence>
        ...
        <div className="flex items-start justify-between mb-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#9898A8] font-medium">
              Heart Rate
            </span>
          </div>
          <motion.div
        ...
        <div className="flex justify-between mt-2 shrink-0">
          {["60", "65", "70", "75", "80"].map((v, i) => (
            <span key={i} className="text-[9px] text-[#C4B8D8]">
              {v}
            </span>
          ))}
        </div>
                ...
                <motion.circle
                  ...
                  animate={
                    isHov
                      ? {
                          strokeWidth: 11,
                          filter: `drop-shadow(0 0 6px ${ring.color}99)`,
                        }
                      : {}
                  }
                />
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-bold text-[#0D0D0D]">
                  {ring.value}
                  <span className="text-[#9898A8] font-normal ml-0.5">
                    {ring.unit}
                  </span>
                </span>
              </div>
              ...
              <div
                ...
                style={{ background: ring.color + "18" }}
              >
                <motion.div
    ...
    >
      <div className="flex items-center justify-between mb-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          {[
            { label: "Deep", color: "#7B00CC" },
            { label: "REM", color: "#AA00FF" },
            { label: "Light", color: "#E8D5FF" },
          ].map(({ label, color }) => (
        ...
        {SLEEP_DATA.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="w-full flex flex-col gap-0.5 rounded-t-lg overflow-hidden"
                style={{ height: "100%" }}
              >
                <motion.div
                  className="w-full rounded-sm mt-auto"
                  style={{
                    background: isHov ? "#D8C4F8" : "#EDE5FA",
                    minHeight: 2,
                  }}
                  initial={{ height: 0 }}
                ...
                <motion.div
                  className="w-full"
                  style={{
                    background: isHov ? "#BB44FF" : "#AA00FF",
                    minHeight: 2,
                  }}
                  initial={{ height: 0 }}
                ...
                <motion.div
                  ...
                  style={{
                    background: isHov ? "#9922EE" : "#7B00CC",
                    minHeight: 2,
                  }}
                  initial={{ height: 0 }}
              ...
              <span className="text-[9px] text-[#C4B8D8] mt-1 shrink-0">
                {d.day}
              </span>
            </div>
          );
    ...
    >
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div className="flex items-center gap-2">
          <div
            className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
            style={{ background: "rgba(139,0,220,0.10)" }}
          >
        ...
        <span className="text-xs font-bold" style={{ color: "#8B00DC" }}>
          {done}/{MEDS.length} taken
        </span>
      </div>

      <div
        className="h-1.5 rounded-full mb-4 overflow-hidden shrink-0"
        style={{ background: "rgba(139,0,220,0.08)" }}
      >
        <motion.div
          className="h-full rounded-full"
        ...
        {MEDS.map((med, i) => (
          ...
          >
            <AnimatePresence>
              {burst === i && (
                <motion.div
                  ...
                  style={{
                    background:
                      "radial-gradient(circle, rgba(139,0,220,0.25), transparent 65%)",
                  }}
                />
              )}
            </AnimatePresence>
            ...
            <div className="flex-1 min-w-0">
              <div
                className={`text-sm font-semibold truncate ${checked[i] ? "line-through text-[#C4B8D8]" : "text-[#0D0D0D]"}`}
              >
                {med.name}
              </div>
...
function AIScoreWidget() {
  ...
  return (
    <TiltCard
      ...
      style={{
        background:
          "linear-gradient(160deg, white 60%, rgba(139,0,220,0.03) 100%)",
      }}
    >
      <div className="flex items-center gap-2 mb-4 shrink-0">
        <div
          className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
          style={{
            background: "linear-gradient(135deg, #7B00CC22, #CC00FF22)",
          }}
        >
          ...
          >
            ...
            >
              <motion.div
                className="h-full rounded-full"
                style={{
                  background: `linear-gradient(90deg, ${dim.color}, ${dim.color}99)`,
                }}
                initial={{ width: 0 }}
