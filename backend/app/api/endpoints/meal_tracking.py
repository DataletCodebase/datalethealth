...
# ====================================================
# ✅ Skip with alternate food — JSON body (no photo)
#    Use this for Postman raw JSON / Android integration
#    POST /meal-tracking/skip-with-food-json
# ====================================================

@router.post("/skip-with-food-json")
async def skip_with_food_json(
    ...
    try:
        ...
        _resp = _client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": f"Estimate calories for: {food_name}. Reply ONLY with a number."}],
            temperature=0
        )
        _nums = _re.findall(r"\d+", _resp.choices[0].message.content.strip())
        calories = int(_nums[0]) if _nums else 0
    except Exception as e:
        print(f"AI calorie estimate error: {e}")
...
# ====================================================
# ✅ Skip with alternate food — form-data (with photo)
#    POST /meal-tracking/skip-with-food
# ====================================================
